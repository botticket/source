    ...
    if 'IQXUSTB' in text_from_user:
        ...
        def usdcheck():
            ...
            line_bot_api.reply_message(
                    event.reply_token,
                    messages=[text_to_reply1,text_to_reply2]
                )
        ...
        class stock:
            ...
            def ticket(self):
                ...
                text = f'{list}' + ' : ' + Close + ' ({})'.format(endday) +'\n' + 'B ' + stop + ' ~ '+ buy1 +'\n' + 'X ' + exit1 + ' | ' + exit2 + ' | ' + exit3
                text2 = 'มีแรงขาย' + '\n'  +f'{list}' + ' : ' + Close + ' ({})'.format(endday) +'\n' + 'B ' + stop + ' ~ '+ buy1
...
def RegisRichmenu(event):
    userid = event.source.user_id
    disname = line_bot_api.get_profile(user_id=userid).display_name
    line_bot_api.link_rich_menu_to_user(userid,'richmenu-0fab92374e1c7886b44799343f007708')
